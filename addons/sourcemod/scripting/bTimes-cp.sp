...
public Menu_DeleteRange1(Handle:menu, MenuAction:action, param1, param2)
{
	if(action == MenuAction_Select)
	{
		...
		{
			if(g_range_one == g_cpcount[param1] - 1)
			{
		...
		{
			if(g_range_one <= g_cpcount[param1] - 1 && g_range_one >= g_cpcount[param1] - 10)
			{
				PrintColorText(param1, "%s%sCannot have CP1 equal to highest CP.", 
				g_msg_start, 
				g_msg_textcol);
		...
		{
			if(g_range_one >= 1 && g_range_one < 11)
			{
				PrintColorText(param1, "%s%sTrying to delete lower than 1 CP.", 
				g_msg_start, 
				g_msg_textcol);
...
public Menu_DeleteRange2(Handle:menu, MenuAction:action, param1, param2)
{
	if(action == MenuAction_Select)
	{
		...
		if (StrEqual(info, "add1"))
		{
			if(g_range_two == g_cpcount[param1])
			{
		...
		if (StrEqual(info, "add10"))
		{
			if(g_range_two <= g_cpcount[param1] - 1 && g_range_two >= g_cpcount[param1] - 9 || g_range_two == g_cpcount[param1])
			{
				PrintColorText(param1, "%s%sCannot have CP2 higher than highest CP.", 
				g_msg_start, 
				g_msg_textcol);
		...
		if (StrEqual(info, "remove10"))
		{
			if(g_range_two >= 1 && g_range_two < 11)
			{
				PrintColorText(param1, "%s%sTrying to delete lower than 1 CP.", 
				g_msg_start, 
				g_msg_textcol);
		...
		if (StrEqual(info, "confirm"))
		{
			...
			{
		 for (new i = g_range_one; i <= g_range_two; ++i)
		 {
    	DeleteCheckpoint(param1, g_range_one);
    }
    OpenCheckpointMenu(param1);
    g_range_one = 0;
    g_range_two = 0;
